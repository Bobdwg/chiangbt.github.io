...
L.TileLayer.HeatMap = L.TileLayer.Canvas.extend({
	...
	calRadius : function(map, value, options) {
		var _getLngRadius = function(point) {
			return _getLatRadius() / Math.cos(L.LatLng.DEG_TO_RAD * point.lat);
		};

		var _getLatRadius = function() {
			return (value / 40075017) * 360;
		};

		var projectLatlngs = function(point) {
			var lngRadius = _getLngRadius(point), latlng2 = new L.LatLng(point.lat, point.lng - lngRadius, true), p = map.latLngToLayerPoint(latlng2), q = map.latLngToLayerPoint(point);
			return Math.max(Math.round(q.x - p.x), 1);
		};
		var centerPoint = new L.Point(options.tileSize / 2, options.tileSize / 2);
		var p = map.unproject(centerPoint);
		return projectLatlngs(p);
	},
	...
	_isInTile : function(localXY, padding) {
		...
		if (!bounds) {
			var tileSize = this.options.tileSize;
			var p1 = new L.Point(-padding, -padding);
			//topLeft
			var p2 = new L.Point(padding + tileSize, padding + tileSize);
			//bottomRight
	...
	_getMaxValue : function() {
		if (this.options.maxPerView) {
			...
			this._data.forEach(function(item) {
				if (mapBounds.contains([item.lat, item.lng])) {
					dataset.push(item);
				};
			});
	...
	_draw : function(ctx) {
		...
		if (this._data.length > 0) {
			for (var i = 0, l = this._data.length; i < l; i++) {
				...
				var lonlat = [this._data[i].lng, this._data[i].lat];
